...
| `dashboard/qa-reporting-dashboard.html` | The single-file static dashboard - hand-authored UI (HTML/CSS/JS) committed and edited directly, PLUS two embedded data consts (`REAL_BIRTH_REG_DATA`/`REAL_CP_DATA`) regenerated in place by `dashboard/embed_dashboard_data.py`. Since `plans/publishing-and-history.md` Phase 3 (2026-09-16, Keith's explicit call): **only `.github/workflows/deploy-pages.yml` ever commits a change to those two consts** - it rebuilds the whole dashboard from committed `qa_results/` history on every relevant push (no real tool re-run, no manually-committed pre-built artifact - CI is the only publish path, per Thread A), gates the result (structural + check-lifecycle + real-browser render checks - see `qa_tools/common/validate_check_lifecycle.py`/`dashboard/check_dashboard_renders.py`), and only then commits+pushes+deploys. A local `./run_pipeline.sh` run still regenerates this file in place for your own viewing (same as always) - **don't commit that regeneration yourself**; let CI's own rebuild be the thing that lands in git, so what's published always traces back to an actual `qa_results/`/check-definition commit, not a snapshot of whatever happened to be on one person's machine. |
