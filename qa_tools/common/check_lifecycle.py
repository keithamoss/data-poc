...
own naming). A check with no `check_id` in its metadata is invisible to
this module entirely - not an error, just not yet migrated (Phase 1's
retrofit is what adds check_id to every real check that exists today).
...
def _parse_dbt_test(test: Any, source: str) -> list[CheckMetadata]:
    if isinstance(test, str):
        return []  # a bare test name (e.g. `unique`) - no config/meta block possible
    ...
    if not check_id:
        return []  # not yet migrated - not an error, just invisible to this module
...
def parse_dbt_check_metadata(schema_yml_path: Path | str) -> list[CheckMetadata]:
    ...
    for model in doc.get("models", []) or []:
        for test in model.get("tests", []) or []:  # model-level (dbt_utils.expression_is_true etc.)
            out.extend(_parse_dbt_test(test, source))
...
def _parse_soda_check(check: Any, source: str) -> list[CheckMetadata]:
...
def parse_soda_check_metadata(soda_yml_path: Path | str) -> list[CheckMetadata]:
    ...
    for key, checks in doc.items():
        ...
        for check in checks or []:
            out.extend(_parse_soda_check(check, source))
    return out
...
def _parse_contract_quality_rule(rule: dict, source: str) -> list[CheckMetadata]:
    rule = dict(rule)
...
def parse_contract_check_metadata(contract_yaml_path: Path | str) -> list[CheckMetadata]:
    ...
    for table in doc.get("schema", []) or []:
        for rule in table.get("quality", []) or []:  # table-level quality rules
            out.extend(_parse_contract_quality_rule(rule, source))
