...
CSV, this reads all 6 real tables directly from cp_raw_dir, cp_duckdb_dir/<run_id>/ -
...
def _run(monkeypatch, cp_raw_dir, cp_duckdb_dir, run_id, run_timestamp):
    monkeypatch.setattr(run_datacontract_cp, "CP_RAW_DIR", cp_raw_dir, cp_duckdb_dir)
...
def test_clean_run_resolves_every_check_id_across_all_6_tables(monkeypatch, cp_raw_dir, cp_duckdb_dir):
    results = _run(monkeypatch, cp_raw_dir, cp_duckdb_dir, _REF_RUN_ID, "2026-01-01T09:00:00Z")
...
def test_dirty_run_produces_a_real_failure(monkeypatch, cp_raw_dir, cp_duckdb_dir):
    results = _run(monkeypatch, cp_raw_dir, cp_duckdb_dir, _DIRTY_RUN_ID, "2026-04-01T09:00:00Z")
...
def test_every_sql_rule_routes_to_a_real_column(monkeypatch, cp_raw_dir, cp_duckdb_dir):
...
def test_a_sql_rules_name_is_authored_not_taken_from_its_description(monkeypatch, cp_raw_dir, cp_duckdb_dir):
...
def test_a_range_check_is_not_labelled_referential_integrity(monkeypatch, cp_raw_dir, cp_duckdb_dir):
