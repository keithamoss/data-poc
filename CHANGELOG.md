...
- **10:16am** — **A Generated Index of the Project's Own Planning Memory** **[Docs & process]** **[Testing & dev tooling]**
  ...
  **159,000 tokens before any work begins**. The awkward part is that
  it isn't dead weight — those completed write-ups are exactly what
  stops a session re-deriving a settled decision, which is the
  instruction's own stated reason for existing.
  So nothing is cut. `plans/INDEX.md` is a **derived view** — one line
  per entry, ~4,400 tokens, generated by
  `mothman dashboard plans-index` from the same parser the dashboard's
  Plans tab uses. Every entry it lists still exists in full in its own
  file, and the index says so in its own header. Generated rather than
  hand-written because a hand-maintained summary of 150+ entries drifts
  the moment anyone is in a hurry, and a CI gate
...
- **10:16am** — **Thirteen Plans Items Were Invisible to the Plans Tab** **[Dashboard UI]**
  ...
  never reached the dashboard's Plans tab, which presents itself as the
  ...
  HIGH-priority per-dataset architecture work. Found while building the
  index, which surfaced it immediately by showing a file with threads
  and no items. The regression test asserts generically that no item in
  any `plans/*.md` goes unparsed, rather than checking the list, so the
