...
**TS-3 `[INJECT]` Arrival just OUTSIDE the claim window.**
*Config: quarterly, calendar Feb 2 / May 1 / Aug 3, early window 14
days. Supply arrives 19 July - one day outside August's window, which
opens 20 July.*
Reframed from daily during Keith's review: the original example (22:00
due, 21:57 arrival) does not work, because the claim window opens BEFORE
due_at, so 21:57 sits comfortably inside it and files to Tuesday
correctly. The scenario only bites where the gap genuinely exceeds the
window, which a quarterly cadence makes realistic.
**Expect**, and both branches matter (Keith asked which applies):
- **May's slot FILLED** - August is not claimable, May and Feb are
  filled, so no claimable unfilled slot exists -> **resupply of May**,
  which does NOT auto-promote and warns.
- **May's slot UNFILLED** - May is claimable -> fills **May, ~79 days
  late**.

**What this test is FOR, and it is a different kind from the rest.**
Most scenarios here assert the system gets the right answer. This one
asserts that **a known imperfection stays contained**:
1. The claim window gates exactly where config says - an arrival one day
   outside does not reach forward into August. Never-claim-forward under
   test at its own boundary, which is the property both cascades rest
   on.
2. The filled-slot branch proves the rules COMPOSE - a misfiling cannot
   silently promote.
3. The unfilled branch is not an imperfection at all: if May never
   arrived and something turns up in July, reading it as May's very late
   supply is entirely reasonable.
Label it as such, so a later session does not "fix" it.
...
Viewing as at 15 September, August data is current - Q4 is not due yet -
and green is correct. Implement it against "today" and every historical
view goes falsely stale, which trains people to ignore the indicator.
